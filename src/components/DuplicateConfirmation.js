...
class DuplicateConfirmation extends React.Component {
	...
	componentDidMount() {
		...
		campaignGroups.page.docs.forEach((campaignGroup, index) => {
			if (selectedGroupIdsArr.includes(campaignGroup._id)) {
				...
				initialProps['name_' + campaignGroup._id] = campaignGroup.name.split("_")[0] + '_' + moment().unix();
				initialProps['groupType_' + campaignGroup._id] = campaignGroup.groupType?campaignGroup.groupType:"budgeted";
				let pvr=campaignGroup.totalBudgetAmount / calendar[0].brands[campaignGroup.brand.name]
				if (campaignGroup.groupType && campaignGroup.groupType === "per_unit") {
					pvr = campaignGroup.pvr
					initialProps['budget_' + campaignGroup._id] = calendar[0].brands[campaignGroup.brand.name] * campaignGroup.pvr;
				} else {
	...
	submit = (values) => {
		...
		let groupsDataTobeDuplicated = selectedGroupIds.map((group) => {
			return {
				groupId: group.id,
				name: values['name_' + group.id],
				//totalBudgetAmount: values['pvr_' + group.id] * values['volumeGoal_' + group.id],
				totalBudgetAmount: values['budget_' + group.id],
				capValue: values['cap_' + group.id],
				startDate: values['calendarMonth_' + group.id] && moment(values['calendarMonth_' + group.id].split('-')[0], 'MM-DD-YYYY'),
				endDate: values['calendarMonth_' + group.id] && moment(values['calendarMonth_' + group.id].split('-')[1], 'MM-DD-YYYY'),
				pvr: values['pvr_' + group.id],
				volumeGoal: values['volumeGoal_' + group.id],
			}
		});

		this.props.duplicateGroups({duplicationArray: groupsDataTobeDuplicated});
	}
	...
	handleCalendarMonthChange = (event, groupId) => {
		...
		let currentCampType = this.props.campForm['campaigns-groups-to-duplicate']['values']['groupType_' + groupId]
		if (currentCampType === "budgeted") {
			let budget = this.props.campForm['campaigns-groups-to-duplicate']['values']['budget_' + groupId]
			this.props.change('pvr_' + groupId, budget / volumeGoal);
			this.props.change('cap_' + groupId, budget / volumeGoal);
		} else {
			let pvr = this.props.campForm['campaigns-groups-to-duplicate']['values']['pvr_' + groupId]
	...
	handleBudgetChange = (event, groupId) => {
		const budget = this.props.campForm['campaigns-groups-to-duplicate']['values']['budget_' + groupId]
		const volumeGoal = this.props.campForm['campaigns-groups-to-duplicate']['values']['volumeGoal_' + groupId]
		this.props.change('pvr_' + groupId, budget / volumeGoal);
		this.props.change('cap_' + groupId, budget / volumeGoal);
	}

	handlePvrChange = (event, groupId) => {
		const pvr = this.props.campForm['campaigns-groups-to-duplicate']['values']['pvr_' + groupId]
		const volumeGoal = this.props.campForm['campaigns-groups-to-duplicate']['values']['volumeGoal_' + groupId]
		this.props.change('budget_' + groupId, pvr * volumeGoal);
...
DuplicateConfirmation = reduxForm({
	form: 'campaigns-groups-to-duplicate',
	enableReinitialize: true,
})(DuplicateConfirmation);

export default connect(mapStateToProps)(DuplicateConfirmation);
